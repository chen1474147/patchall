...
void getleft_b_e0(Mat &Src, Mat &Mask, int **m_c, int *m_c_c) {
	int max_seg = 20;
	//ÿһ����������
	int min_seg = 0;
...
//������
void copy0(Mat &Src, Mat &h, Mat &Mask) {
	...
	getleft_b_e0(Src, Mask, m_c, m_c_c);
...
//������
void copy20(Mat &Src, Mat &h, Mat &Mask) {
...
void alllarge0(string n)
{
	...
	for (int seg = 0; seg< allseg; seg++)
	{
		...
		string input = "expand2/";
		string output = "expand3/";

		string name = n;

		string texturename = "_texture_";
		string maskname = "_mtexture_";
		...
		{
			Mat Src2 = Mat(Src.cols, Src.rows, CV_8UC3, Scalar::all(0));
			Mat Mask2 = Mat(Src.cols, Src.rows, CV_8UC3, Scalar::all(0));
			for (int i = 0; i< Src.rows; i++)
				for(int j = 0; j< Src.cols; j++)
				{
					Src2.at<Vec3b>(j, i) = Src.at<Vec3b>(i, j);
					Mask2.at<Vec3b>(j, i) = Maskinfo.at<Vec3b>(i, j);
				}
			...
			for (int i = 0; i< Src.rows; i++)
				for(int j = 0; j< Src.cols; j++)
				{
					Src.at<Vec3b>(i, j) = Src2.at<Vec3b>(j, i);
					Maskinfo.at<Vec3b>(i, j) = Mask2.at<Vec3b>(j, i);
				}
