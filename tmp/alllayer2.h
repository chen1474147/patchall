...
void alllayer(string n, int le, int se)
{
	...
	for (int segnum = 0; segnum < allsegnum; segnum++)
	{
		...
		if (!Src.empty() && !Maskinfo.empty()) {
			...
			//向上，左，右分别扩大50个像素
			Mat newSrc = Mat(oldrow + addpixels, ollcol + 2 * addpixels, CV_8UC3,
					Scalar::all(0));
			Mat newMask = Mat(oldrow + addpixels, ollcol + 2 * addpixels, CV_8UC3,
					Scalar::all(0));

			for (int i = 0; i < oldrow; i++) {
				for (int j = 0; j < ollcol; j++) {
					newSrc.at<Vec3b>(i + addpixels, j + addpixels) = Src.at<Vec3b>(i, j);
					newMask.at<Vec3b>(i + addpixels, j + addpixels) = Maskinfo.at<Vec3b>(i,
							j);
				}
			}
			...
			{
				//背景
				Mat newMask1 = Mat(oldrow + addpixels, ollcol + 2 * addpixels, CV_8UC1,
						Scalar::all(search));
				...
				*/
				...
				for (int i = 0; i < oldrow + addpixels; i++) {
				...
				for (int i = 0; i < oldrow+addpixels; i++) {
					for (int j = ollcol+addpixels; j < ollcol+2*addpixels; j++) {
						newMask1.at<uchar>(i, j) = hole;
					}
				}
	...
	for (int ll = 0; ll<= allayernum; ll++)
	{
		...
		Mat newLayer = Mat(Layer.rows+addpixels, Layer.cols+2*addpixels, CV_8UC3, Scalar::all(0));
		for (int i = 0; i < Layer.rows; i++)
		{
			for (int j = 0; j < Layer.cols; j++)
			{
				newLayer.at<Vec3b>(i+addpixels, j + addpixels) = Layer.at<Vec3b>(i, j);
			}
		}
		...
		for (int j = 0; j < layerobjcount[ll] ; j++)
		{
			...
			if (!Src.empty() && !Maskinfo.empty())
			{
				//imshow("obj", Src);
					...
					for (int i = 0; i < addpixels; i++)
					{
						for (int jj = 0; jj < Maskinfo.cols; jj++)
						{
							if (Maskinfo.at<Vec3b>(i, jj) != Vec3b(0, 0, 0))
							{
								newLayer.at<Vec3b>(i, jj) = Vec3b(255, 255, 255);
